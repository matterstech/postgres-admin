...
COMMENT ON FUNCTION array_avg(anyarray) IS 'Compute the average of an array';-- based on query from http://wiki.postgresql.org/wiki/Disk_Usage
CREATE VIEW database_size AS
    ...
    ORDER BY
        db_size DESC
;
COMMENT ON VIEW database_size IS 'List all databases and their disk usage';CREATE VIEW extension_object AS
    SELECT
...
;
COMMENT ON VIEW extension_object IS 'List of all object packed in an extension with associated comment';CREATE OR REPLACE FUNCTION get_function_def_list(IN schema_search TEXT DEFAULT 'public', OUT definition TEXT) RETURNS SETOF TEXT
AS
$BODY$
DECLARE
  query TEXT;
...
COMMENT ON FUNCTION get_function_def_list(TEXT) IS 'Get the list of functions for a specif schema including their comments. If a function does not have a comment it will not be pull out';-- based on query from http://wiki.postgresql.org/wiki/Index_Maintenance
CREATE VIEW index_duplicate AS
    ...
    ORDER BY
        duplicate_id
;
COMMENT ON VIEW index_duplicate IS 'List all indexes similar to each other, you should keep an eye on those indexes';CREATE VIEW index_operator AS
    ...
    ORDER BY
        ...
        index_name
;
COMMENT ON VIEW index_operator IS 'List of all valid operators for an index';-- based on query from http://wiki.postgresql.org/wiki/Index_Maintenance
CREATE VIEW index_usage AS
    ...
    ORDER BY
        ...
        table_name
;
COMMENT ON VIEW index_usage IS 'List all indexes and index usage statistics, easily find unused indexes';-- Test if an argument is a float
CREATE FUNCTION is_float(data_to_test text, separator text DEFAULT '.'::text)
  RETURNS boolean AS $$
  SELECT data_to_test ~ ('^[+-]?\d+(\'|| separator || '\d+)?$')
$$
LANGUAGE SQL IMMUTABLE;
COMMENT ON FUNCTION is_float(data_to_test text, separator text) IS 'Test if a value is in fact a float';-- Test if an argument is an integer
CREATE FUNCTION is_int(data_to_test text) RETURNS boolean AS $$
	SELECT data_to_test ~ '^[0-9]+$'
$$
LANGUAGE SQL IMMUTABLE;
COMMENT ON FUNCTION is_int(data_to_test text) IS 'Test if a value is in fact an integer';-- based on query from https://wiki.postgresql.org/wiki/Server_Configuration
...
;
COMMENT ON VIEW setting_delta IS 'List of settings that have been changed from the default by any source';CREATE VIEW table_size AS
select
